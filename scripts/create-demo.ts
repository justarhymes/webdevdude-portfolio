...
import { DemoInput } from "@/domain/demo";
import { createDemo } from "@/repositories/demoRepository";
...
async function main() {
  ...
  const parsed = DemoInput.safeParse(payload);
  if (!parsed.success) {
  ...
  const result = await createDemo(parsed.data, { dryRun, upsert, allowNew });
  if (!result.ok) {
  ...
  if (dryRun) {
    ...
    console.log("Created/Updated:", result.data.slug || result.data._id || "OK");
  }
}
